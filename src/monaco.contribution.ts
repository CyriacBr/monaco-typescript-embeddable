...
function createAPI(): typeof monaco.languages.typescript {
  return {
    ...
    setupEmbedded: setupEmbeddedTypeScript
  };
}
...
function setupEmbeddedTypeScript(myLang: string) {
  getMode().then(mode => mode.setupEmbeddedTypeScript(typescriptDefaults, myLang));
}
